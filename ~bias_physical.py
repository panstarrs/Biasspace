...
lbands = ['mstars_tot','mcold','mstardot','L_tot_Halpha','L_tot_Lyalpha','L_tot_Hbeta','mag_GALEX-FUV_r_tot','mag_GALEX-NUV_r_tot','mag_SDSS-u_r_tot','mag_SDSS-g_r_tot','mag_SDSS-r_r_tot','mag_SDSS-i_r_tot','mag_UKIRT-H_r_tot','mag_UKIRT-J_r_tot','mag_UKIRT-K_r_tot']
labels = ['mstar','mcold','sfr','Halpha','Lyalpha','Hbeta','FUV', 'NUV', 'u', 'g', 'r', 'i','H','J','K']
latexs = ['$M_{\star}$','$SFR$','$M_{gas}$','$H_{\ alpha}$','$Ly_{\ alpha}$','$H_{\ beta}$','FUV', 'NUV', 'u', 'g', 'r', 'i','H','J','K']
...
for index, iband in enumerate(lbands):
  ...
  for i in range(len(snapnum)):
    ...
    stbs = np.loadtxt(indirs+'/B%s/G_%s/%s/p_c/%s_tb.txt'%(snapnum[i],db,cso,iband))
    spbs = np.loadtxt(indirs+'/B%s/G_%s/%s/p_c/%s_pb.txt'%(snapnum[i],db,cso,iband))
    ...
    if snapnum[i] == 199 :
      ...
      ax.plot(xdata, physical_func((xdata,np.full(len(xdata),z)), *popt), color=sncolors[i], label='best fit',linewidth=linew[i])
    else :
      ...
      ax.plot(xdata, physical_func((xdata,np.full(len(xdata),sn_to_z(snapnum[i]))), *popt), color=sncolors[i],linewidth=linew[i])

  ax.set_ylim(0.5, 1.1*ymax)
