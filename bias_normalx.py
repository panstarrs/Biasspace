...
#fit function
#fitfile = '/Users/hengxingpan/Work/danail/ProCorr/Bias/output'+'/%s_%s.txt'%(bands,dl)
#if os.path.isfile(fitfile):
#  fit = genfromtxt(fitfile)
#  p0 = fit[:4]
#  popt, pcov = curve_fit(func, (spb[:,0], np.full(len(spb[:,0]),z)), spb[:,1], p0)
#else:
#  popt, pcov = curve_fit(func, (spb[:,0], np.full(len(spb[:,0]),z)), spb[:,1])
...
#np.savetxt(f_handle, popt,fmt='%e')
