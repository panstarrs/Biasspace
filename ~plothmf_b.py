...
ax.set_ylim((0.8,4))
ax.set_ylabel(r'$dn/dn_{Vhalo_F}$')
...
zx = 0.1
zy = np.array([0.6,0.5,0.4,0.3])+0.5
...
for i in range(len(snapnum)):
  ...
  hmfdat[0] = hmfdat[0][:17,:]
  hmfdat[1] = hmfdat[1][:17,:]
  hmfdat[2] = hmfdat[2][:17,:]

  ax.annotate(zlabels[i], xy=(zx, zy[i]-0.4), xycoords="axes fraction",color=sncolors[i],size=10)

  for j in range(len(labels)):
    if i==0 :ax.plot(10**hmfdat[j][:,0],hmfdat[j][:,1]/hmfdat[0][:,1],label=labels[j],lw=1,color=sncolors[i], linestyle=linestyles[j])
    #print i,j,len(hmfdat[j][:,1])
    ax.plot(10**hmfdat[j][:,0],hmfdat[j][:,1]/hmfdat[0][:,1],lw=1,color=sncolors[i], linestyle=linestyles[j])
...
fit = genfromtxt('./output/Fit/%s_%s.txt'%(bands,dh))
print '%s'%(bands)
